...
export function StopCard({
  ...
  async function bookClick() {
    ...
    if (stop.bookingUrl) window.open(stop.bookingUrl, "_blank");
    ...
    >
      ...
      {stop.warnings.length > 0 && (
        <ul className="mt-3 flex flex-wrap gap-2">
          {stop.warnings.map((w) => (
            <li
              key={w}
              className="rounded-full bg-yellow-100 px-2 py-0.5 text-xs text-yellow-900"
            >
              {w}
            </li>
          ))}
      ...
      {stop.bookable && (
        <button
          ...
          className="mt-4 rounded-full bg-ink px-4 py-1.5 text-xs text-sand hover:opacity-90"
        >
          Book / enquire
        </button>
      )}
