...
export function AnchorEditor({ imageUrl, width, height, initialAnchors }: Props) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="lg:col-span-2 overflow-hidden rounded border border-ink/10 bg-white">
        <svg
          ref={svgRef}
      ...
      <aside className="space-y-4">
        <div className="rounded border border-ink/10 bg-white p-4">
          ...
          {errors.length > 0 && (
            <ul className="mt-2 max-h-40 overflow-y-auto text-xs">
              {errors.map((e) => (
                <li key={e.index} className="flex justify-between text-ink/70">
        ...
        <div className="rounded border border-ink/10 bg-white p-4 space-y-2">
          <h2 className="mb-1 font-medium">Add anchor</h2>
          {pending ? (
            <p className="text-xs text-ink/60">Selected: {pending.x.toFixed(0)}, {pending.y.toFixed(0)}</p>
          ) : (
            <p className="text-xs text-ink/50">Click a point on the map…</p>
          )}
          <input placeholder="Label" value={label} onChange={(e) => setLabel(e.target.value)} className="w-full rounded border border-ink/20 px-2 py-1 text-sm" />
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="Lat" value={lat} onChange={(e) => setLat(e.target.value)} className="rounded border border-ink/20 px-2 py-1 text-sm" />
            <input placeholder="Lng" value={lng} onChange={(e) => setLng(e.target.value)} className="rounded border border-ink/20 px-2 py-1 text-sm" />
          </div>
          <button onClick={addAnchor} disabled={!pending || !lat || !lng} className="w-full rounded bg-ink py-1.5 text-sm text-sand disabled:opacity-40">
            Add
          </button>
        </div>
          ...
          <ul className="space-y-1 text-xs">
            {anchors.map((a) => (
              <li key={a.id} className="flex items-center justify-between">
                ...
                <button onClick={() => removeAnchor(a.id)} className="text-coral">remove</button>
              </li>
            ))}
          </ul>
        </div>

        <button onClick={download} className="w-full rounded bg-ink px-3 py-2 text-sm text-sand hover:opacity-90">
